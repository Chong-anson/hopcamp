...
export const filterCampsites = (state) => {
  ...
  if (state.ui.filter.appliedFilter || selectedCampsites.length)
    campsites = campsites.filter((campsite) =>
      selectedCampsites.includes(campsite.id)
    );
  if (type.length)
    campsites = campsites.filter((campsite) =>
      type.includes(campsite.campsiteType)
    );
