...
const msp = (state, ownProps) => {
  ...
  const { checkedTags, appliedFilter, minCapacity, maxPrice } = state.ui.filter;
  if (tags.length) {
    // categorize tags
    const categorized = {};
    tags.forEach((tag) => {
      if (categorized[tag.category]) categorized[tag.category].push(tag);
      else categorized[tag.category] = [tag];
    });
    return {
      categorized,
      campsites,
