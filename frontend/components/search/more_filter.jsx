...
class MoreFilter extends React.Component {
  ...
  handleSizeChange(e) {
    ...
    const selectedCampsites = this.state.selectedCampsites.filter(
      (campsite) => campsite.capacity >= minCapacity
    );
    this.setState({ selectedCampsites, minCapacity });
  }

  handlePriceChange(e) {
    const maxPrice = parseInt(e.target.value);
    const selectedCampsites = this.state.selectedCampsites.filter(
      (campsite) => campsite.price <= maxPrice
    );
    this.setState({ selectedCampsites, maxPrice });
  }
  ...
  handleSubmit(e) {
    e.preventDefault();
    const { appliedFilter, checkedTags, minCapacity, maxPrice } = this.state;
    const selectedCampsites = this.state.selectedCampsites.map(
  ...
  render() {
    ...
    if (this.props.categorized) {
      ...
      Object.keys(this.props.categorized).forEach(
        (key) =>
          (categorizedList[key] = Object.values(
            this.props.categorized[key]
          ).map((tag) => {
            let className = "normal";
            let icon = null;
            if (
              ["Amenities", "Terrain"].includes(tag.category) ||
              ["Hiking", "Swimming"].includes(tag.name)
            ) {
              className = "special";
              icon = (
                <span
                  className={`more-filter-icon 
                    hc-awesome-${[tag.icon]}`}
                ></span>
              );
            }
