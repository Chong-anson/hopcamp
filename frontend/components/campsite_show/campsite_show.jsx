...
class CampsiteShow extends React.Component {
  ...
  componentDidMount() {
    this.props
      .fetchCampsite(this.props.match.params.id)
      .then(() => this.fetchGeoInfo());
  }
  ...
  render() {
    ...
    if (this.props.campsite && !this.props.campsite.truncated) {
      ...
      const campsiteDescription = truncated
        ? campsite.description.slice(0, charIndex)
        ...
        if (tag) {
          tags[tag.category].push(
            <li key={`${tag.category}-${idx}`}>
              <span
                className={`filter-type-icon hc-awesome-${tag.icon}`}
              ></span>
              <p> {tag.name}</p>
            </li>
          );
            ...
            <div className="campsite-show-info">
              <div className="campsite-show-title">
                ...
                <div className="campsite-title">
                  <h1>{campsite.name}</h1>
                  <span
                    className={`hc-awesome-${campsite.campsiteType.toLowerCase()}`}
                  ></span>
                </div>
