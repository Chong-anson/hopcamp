...
const ReviewForm = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (type === "create") {
      dispatch(createReview(reviewForm)).then(() => {
        closeForm();
        dispatch(clearReviewErrors());
      });
    } else {
      reviewForm.id = review.id;
      dispatch(updateReview(reviewForm)).then(() => {
        closeForm();
        dispatch(clearReviewErrors());
      });
    }
  ...
  return (
    <div className="review-form-container">
      <div className="left">
        <h2>{currentUser ? currentUser.firstName : ""}</h2>
      </div>
      <div className="right">
        <form className="review-form">
          ...
          <button className="special-buttons-2" onClick={handleSubmit}>
            {type === "create" ? "Submit " : "Update "} Review
          </button>
        </form>
